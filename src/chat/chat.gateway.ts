...
import { setInitDTO, chatRoomListDTO } from '../_common/dtos/chatBackEnd.dto';
...
export class ChatBackEndGateway implements OnGatewayConnection, OnGatewayDisconnect {
  constructor(
  ...
  async sendMessage(client: Socket, message: string): Promise<void> {
    ...
    // await this.chatService.saveMessage(roomId, client.id, nickname, message, timestamp);
  ...
  async enterChatRoom(client: Socket, roomId: string): Promise<void> {
    const previousMessages = await this.chatService.getMessages(roomId);
